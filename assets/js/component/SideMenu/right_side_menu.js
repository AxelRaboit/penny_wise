export function initializeRightSideMenu(rightSideMenuConfig) {
    ...
    rightSideMenuConfig.forEach(({ openRightSideMenuButton, contentRightSideMenu, closeRightSideMenuButton, additionalClasses = '' }) => {
        ...
        if (openButton && content && closeButton) {
            ...
            buttonsInsideModal.forEach((button) => {
                button.addEventListener('click', (event) => {
                    if (!button.classList.contains('side-menu-no-close-on-interaction')) {
                        closeRightSideMenu();
                    }
                });
            });
